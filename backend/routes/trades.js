...
router.get('/', (req, res) => {
  ...
  if (status) {
    res.json(db.prepare('SELECT * FROM trades WHERE status = ? AND (user_id = ? OR user_id IS NULL) ORDER BY opened_at DESC').all(status, userId));
  } else {
    res.json(db.prepare('SELECT * FROM trades WHERE (user_id = ? OR user_id IS NULL) ORDER BY opened_at DESC').all(userId));
  }
});

router.post('/', (req, res) => {
  const { pair, direction, quantity, entry_price, tp, sl, entry_amount } = req.body;
  if (!pair || !direction || !entry_price) return res.status(400).json({ error: 'Missing fields' });
  // Calculate quantity from entry_amount if provided, otherwise use quantity directly
  const actualQuantity = entry_amount ? entry_amount / entry_price : quantity;
  if (!actualQuantity) return res.status(400).json({ error: 'Missing quantity or entry_amount' });
  const userId = req.user?.id || null;
  const r = db.prepare('INSERT INTO trades (pair, direction, quantity, entry_price, tp, sl, entry_amount, user_id) VALUES (?,?,?,?,?,?,?,?)').run(pair, direction, actualQuantity, entry_price, tp || null, sl || null, entry_amount || null, userId);
  res.json({ id: r.lastInsertRowid });
});

router.post('/:id/close', async (req, res) => {
  const userId = req.user?.id;
  const trade = db.prepare('SELECT * FROM trades WHERE id = ? AND status = ? AND (user_id = ? OR user_id IS NULL)').get(req.params.id, 'open', userId);
...
router.get('/stats', (req, res) => {
  const userId = req.user?.id;
  const closed = db.prepare('SELECT * FROM trades WHERE status = ? AND (user_id = ? OR user_id IS NULL)').all('closed', userId);
