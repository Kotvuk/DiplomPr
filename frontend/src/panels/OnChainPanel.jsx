    ...
    try {
      ...
      if (r.ok) {
        ...
        if (data && !data.error) {
          ...
          setError('On-chain data unavailable (Glassnode API key required)');
          setMetrics(null);
        }
      } else {
    ...
    <div>
      ...
      {error && (
        <div style={{ color: theme.textMuted, textAlign: 'center', padding: '40px 20px' }}>
          ...
          <div style={{ fontSize: 16, color: theme.text, marginBottom: 8 }}>On-chain данные недоступны</div>
          <div style={{ fontSize: 13 }}>{error}</div>
          <div style={{ fontSize: 12, marginTop: 8, color: theme.textMuted }}>
            Для on-chain метрик требуется Glassnode API ключ
          </div>
        </div>
      )}
